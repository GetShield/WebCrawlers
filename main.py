...
#id = 989483566391160832
#res = client.get_users_tweets(id=id, exclude=['retweets', 'replies'], max_results=5)
...
def getIds():
    for link in links:
        screenName = link.split('/')[-1]
        id = getIdByName(screenName)
        ids.append(id)
...
def fetch_and_store_data(link):
    ...
    try:
        response = client.get_users_tweets(id=id, exclude=['retweets', 'replies'], start_time=startTime)
        ...
        for tweet in tweets:
                    ...
                    for li in content.split("\n"):
                        ...
                        print(lnk)
                        print(f"insert into main(tweet_id,username,name,images,tweet_url,link,profile) VALUES('{status}','{username}','{name}','{[]}','{link}','{lnk}','{link}')")
                        cur.execute(
