...
def crawler():
    ...
    adResults = open("adResults", "a+")


    for word in soegeOrd:

            print(">> Finding search results for:", word)
            results = api.search(word)
            if results == []:
                continue
            adResults.write(word + "\t")
            res = ""
            for result in results:
                url = urlparse(result).netloc
                domainName = '.'.join(url.split('.')[-2])
                if domainName != word: 
                    res +=  result + "\t"
            if res != "":
                data = word + "\t" + res + "\t"
                adResults.write(data)
            adResults.flush()

    adResults.close()
    print(">> Search result are ready in: adResults")
